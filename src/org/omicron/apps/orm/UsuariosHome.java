 ...
 * @see org.omicron.apps.orm.Usuarios
...
public class UsuariosHome {
	...
	public Usuarios findById(java.lang.Integer id) {
		...
		try {
			Usuarios instance = (Usuarios) sessionFactory.getCurrentSession().get("org.omicron.apps.orm.Usuarios", id);
			if (instance == null) {
	...
	public List<Usuarios> findByExample(Usuarios instance) {
		...
		try {
			List<Usuarios> results = (List<Usuarios>) sessionFactory.getCurrentSession()
				.createCriteria("org.omicron.apps.orm.Usuarios").add(create(instance)).list();
